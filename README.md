...
使用跨境魔方 OpenAPI 查找胶带、遮蔽膜、刷子、猪毛刷、羊毛刷、PVC 护角条、塑料桶和油漆刷的海外采购公司，并将公开联系方式维护在一份权威 Excel 客户总表中。

> 第一次接手或需要向公司演示项目，请先阅读：[项目交接与现场演示指南](docs/handoff.md)。

当前主表包含 270 家公司，全部拥有至少一种有效联系方式，平均成本为 ¥1.43/家。其中 205 家拥有有效邮箱，成本为 ¥1.89/家；210 家拥有有效电话，成本为 ¥1.84/家。覆盖结构为：145 家邮箱和电话都有、60 家只有邮箱、65 家只有电话、0 家两者都没有。
...
- 必须显式加 `--execute` 才执行。
- 当前项目累计上限为 ¥500，每个付费脚本都在请求前做预算拦截；当前累计费用为 ¥387.10。
...
`deliverables/客户表（7.17）.xlsx` 是唯一客户数据主账。不创建 `final-v2`、`new`等并行版本；历史通过 Git 追溯。

## 中文展示规范

- 面向业务人员的筛选和理解字段统一优先使用中文：产品品类固定为“胶带”“遮蔽膜”，搜索词使用相同中文口径，国家/地区显示中文名称。
- 产品名称、产品标签、产品别名、上游词和下游词中已识别的英文术语转换为中文；无法可靠翻译的型号、缩写和噪声词保留原样。
- “经营范围（中文）”整列只展示中文；API 原始经营范围继续保存在 `data/processed/company-master.json` 的 `business_scope` 字段和 `data/raw/` 证据中。
- 公司名称、邮箱、电话、网址、报关品名和地址不翻译，保证证据可追溯。
